...
- Prisma Client is automatically connected (singleton pattern)
...
- Use `include` for relationships (replaces populate)
- Dates are automatically serialized as ISO strings
- Indexes defined in `prisma/schema.prisma`

### Data Isolation

- Always filter by `organizationId` when querying
- Never expose data from other organizations
- Compound indexes defined in Prisma schema
